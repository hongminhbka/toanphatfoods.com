...
function themesflat_imagebox_shortcode_params() {
	...
	vc_map( array(
		...
		'params'      => array(
			...
			array(
				'type' => 'checkbox',
				'heading' => esc_html__( 'Button Click Target Blank', 'finance' ),
		...
		<div class="flat-imagebox-inner">
					...
					<?php
						if ( ! $atts['show_button'] ) {
							printf( '<a href="%s" target="%s">%s</a>',
								esc_url( $atts['link'] ), $target, $image );
						}
						else {
							print( $image );
						}						
			...
			<div class="flat-imagebox-header">
				<h3 class="flat-imagebox-title">
					<a href="<?php echo esc_url( $atts['link'] ) ?>" target="<?php echo esc_attr($target) ?>">
